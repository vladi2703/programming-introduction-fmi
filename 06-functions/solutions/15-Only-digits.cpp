...
bool isDigit(char symbol) { return (symbol >= '0' && symbol <= '9'); }
    ...
    if (isDigit(str[i])) {
      ...
      if (flag) {
        for (int k = begin; k <= end;
             k++) { //от поредица от символи '1''2''3' строим цялото число 123
          num = (num * 10) + str[k] -
                '0'; // ако от целочислената репрезентация на цифрата извадим
                     // '0' ще получим стойността на цифрата пример -  int('9')
                     // = 57; int('0') = 48; '9' - '0' = 57 - 48 = 9;
        }
