...
int main() {
  ...
  if (resultFractionPart >= 10 &&
      resultFractionPart % 10 != 0) // дробната част е от две цифри
    std::cout << '.' << resultFractionPart;
  else if (resultFractionPart >= 10 &&
           resultFractionPart % 10 == 0) // една цифра е нужна за дробна част
