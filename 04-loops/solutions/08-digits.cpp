...
int main() {
    ...
    number %= remainder; 
    remainder /= 10; //намаляваме reminder за да можем да вземем следващата позиция
    numberLen--;

    int sum = 0; 
    // Печатаме първата цифра и след това махаме първите 2 цифри, докато числото е > 0
    for (int i = 0; i < numberLen; i += 2) {
        ...
        if (remainder > 0) {
            number %=
                remainder; // ако остатка стане 0 ще се хвърли грешка
        }
